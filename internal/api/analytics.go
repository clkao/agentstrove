// ABOUTME: API handlers for team analytics endpoints (usage, heatmap, tool distribution, tokens-by-model).
...
func (s *Server) handleTokensByModel(w http.ResponseWriter, r *http.Request) {
	q := r.URL.Query()
	...
	result, err := s.store.TokenUsageByModel(r.Context(), "", projectName, dateFrom, dateTo)
	if err != nil {
		writeError(w, http.StatusInternalServerError, err.Error())
		return
	}
