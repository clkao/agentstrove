#!/bin/bash
# ABOUTME: Generate a changelog since the last release using an AI agent.
# ABOUTME: Feeds commit log + diff stat to codex or claude for user-focused summaries.
#
...
cat > "$PROMPTFILE" <<EOF
You are generating a changelog for agentlore version $VERSION.
